...
public class SocialNetwork {
                    ...
                    if (ob == null)
                    {
                        ...
                        for (int j = 0; j < userConnections.length; j++) {
                            ...
                            if (userConnections[j] != null) {
                                ...
                                User connected_user = getUser(userConnections[j].strip());

                                if (connected_user == null) {
                                    User newUser = new User(userConnections[j].strip(), null);
                                    addUser(newUser);
                                    cons[j] = newUser;
                                    // addConnection(u,newUser);
                                }
                                else
                                { // Main user is not present and connection users are not present
                                    User existing=getUser(userConnections[j].strip());
                                    cons[j] = existing;
                                }
                            }
                        ...
                        for (int j = 0; j < userConnections.length; j++) {
                            ...
                            User newUser=getUser(userConnections[j].strip());
                            if(newUser==null) {
                                //System.out.println("New "+userConnections[j].strip());
                                newUser= new User(userConnections[j].strip(), null);
                                cons[j] = newUser;
                                addUser(newUser);
                            }
                            else {
    ...
    public User[] getCommonConnections(User userA, User userB) {
        // TODO
        // Your code goes here
        if(userA==null || userB==null || userB.connections==null || userA.connections==null) {
            //User arr[] = {};
            return null;
        }
